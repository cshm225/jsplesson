...
<body>
<div class="container">
<a href="/donatapp/Main" class="btn btn-outline-info  btn-sm float-right">公開ページを見る</a>
<% if(msg != null){ %>
...
<form class="mt-3" action="/donatapp/Admin" method="post" enctype="multipart/form-data">
...
<%for(Dounut d:list){ %>
<tr>
<td><img src="/donatapp/upload/<%=d.getImgname() %>"></td>
<td><%=d.getId() %></td>
<td><%=d.getName() %></td>
<td><%=d.getPrice() %></td>
<td>
<a href="/donatapp/Admin/Update?id=<%=d.getId() %>">更新</a>
</td>
<td>
<a href="/donatapp/Admin/Delete?id=<%=d.getId() %>" onclick="return confirm('削除してよろしいですか？')">削除</a>
</td>
</tr>
<%} %>
</table>
